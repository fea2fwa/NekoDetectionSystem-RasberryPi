...
while True:
    ...
    if target_object_count >= 1:
        ...
        # 最小通知間隔を超えていた時だけ通知
        if time.time() - last_send_time > MINIMUM_DJANGO_INTERVAL:
            ...
            files = {"detect": (file_name, attached_image, 'image/jpeg')}
            # files = {"detect": (file_name, attached_image, 'rb')}

            # ヘッダとデータ（ボディ）を指定してDJANGO APIサーバにPOSTリクエストを送信
            # res = requests.post(DJANGO_NOTIFY_API, headers=headers, files=files)
            res = requests.post(DJANGO_NOTIFY_API, files=files)
