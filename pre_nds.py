...
# DJANGO関連パラメタ
DJANGO_NOTIFY_API = 'http://160.251.17.38:8000/api/cats'
...
while True:
    ...
    if target_object_count >= 1:
        ...
        if time.time() - last_send_time > MINIMUM_LINE_INTERVAL:
            ...
            # ヘッダとデータ部（ボディ）を指定してDJANGO通知APIサーバにPOSTリクエストを送信
            res = requests.post(DJANGO_NOTIFY_API)
