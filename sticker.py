...
#ステッカーＩＤ　LINE通知ＡＰＩドキュメントのSticker一覧参照
STKID       = 3
STKPKGID    = 1
...
while True:
    ...
    if target_object_count >= 1:
        ...
        if time.time() - last_send_time > MINIMUM_LINE_INTERVAL:
            ...
            # HTTPボディに APIで指定のmessageパラメタとsticker関連パラメタを指定して、通知メッセージをセット
            body = {"message": line_message, "stickerPackageId": STKPKGID, "stickerId": STKID}
